...
async fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    // Send the payload
    let new_result = rpc_call.send("video".to_string(), "fibonacci".to_string(), new_payload).await?;
    ...
    let future_result = rpc_call.call_async("video".to_string(), "hello".to_string(), vec![Value::String("Toto".to_string())]);
    // Send a message
    let result = rpc_call
        .send("video".to_string(), "hello".to_string(), 
            vec![Value::String("Girolle".to_string())],
        )
        .await?;
    // Print the result
    println!("{:?}", result);
    assert_eq!(result, Value::String("Hello, Girolle!, by nameko".to_string()));
    // Wait for the future result
    let mut consumer = future_result.await?;
    // Print the result
    let async_result = rpc_call.result(&mut consumer).await;
    println!("{:?}", async_result);
    assert_eq!(async_result,Value::String("Hello, Toto!, by nameko".to_string()));
    Ok(())
}
